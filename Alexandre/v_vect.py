...
def get_data(data_file_name):
    ...
    Args:
        data_file_name: the string containing the name of the data file
    Return: the list containing the array of mass, positions and velocities
    """

    data= np.loadtxt(data_file_name, dtype=np.double)
    mass=data[:,0]
    color=np.array(gg.generate_star_color(mass) for body_mass in mass)
    position=np.concatenate((data[:,1],data[:,2],data[:,3]), axis=1)
...
def compute_acceleration(mass, position, velocity):
    ...
    Args:
        ...
        position: the positions of the bodies
        velocity: the velocities of the bodies

    Return: 
        gravity : the array of gravity vectors
    """

    gravity=np.zeros((len(mass),3))
    distance=np.array([position[i]-a] for i in range(len(mass)))


def update(delta_t, mass, position, velocity):
    """
    Update the positions and velocities of bodies through gravity
    
    Args:
        delta_t: the step time
        mass: the masses of the bodies
        position: the positions of the bodies
        velocity: the velocities of the bodies

    Return: 
        position : the position array updated
    """
...
def test(data_file_name, delta_t): 
    """
